...
- **右键菜单**：二级菜单设计，一级保留标记优先级/设置项目/删除任务入口
- **任务详情**：点击卡片展开，内置 Vditor 所见即所得 Markdown 编辑器（支持 Typora 风格快捷键）
- **图片附件**：卡片内 Ctrl+V 直接粘贴图片，点击可放大预览
- **时间规划**：设置任务时间、实际开始时间与完成时间，自动计算耗时（开始时间优先，缺省退回任务时间）
- **删除撤销**：删除后 5 秒内可一键撤销

### 多视图
- **日历视图**：按天聚合任务的创建/完成活跃度，点日期可查看当天明细
- **回收站**：软删除任务的集中管理，支持恢复、批量/单条彻底删除，可按保留天数自动清理
- **统计视图**：近 30 天完成/创建趋势、耗时统计、未完成按项目与优先级分布
- **设置视图**：时间范围、侧栏 Tab、日历口径、置顶、小窗尺寸、回收站保留天数等偏好设置
...
- **桌面小窗模式**：默认 380×560（尺寸可在设置页自定义）紧凑小窗自动置顶，「进行中 / 未开始」快速切换，显示项目、时间与优先级色条，点圆圈即可开始/暂停任务
- 保留最小化 / 最大化 / 关闭按钮
...
│       ├── stores/      # Pinia 状态（任务、项目、UI、设置）
│       └── lib/         # Tauri API 封装、窗口控制、耗时格式化
